...
async function mapPriceList(priceJson) {
    const priceList = new hotelPriceList([])
    priceJson["hotels"].map(jsonPrices => {
...
async function mapHotelList(hotelJson, priceList) {
    const hotelListings = new hotelList([])
    hotelJson.map(jsonHotels => {
...
router.get("/:id/:checkin/:checkout/:guests", async function (req, res, next) {
    ...
    if (checkin < currentDate) {
        ...
        if (priceJson["hotels"].length > 0) {
            ...
            const hotelListings = await mapHotelList(hotelJson, priceList)
