  ...
  useEffect(() => {
    if (isGuest) {
      ...
      setError(
        <>
          {" "}
          <FontAwesomeIcon icon={faCircleExclamation} /> Please log in to view
          account information
        </>
      );
      return;
    ...
    const fetchUserDetails = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/account/details",
          {
            params: {
        ...
        setError(
          <>
            {" "}
            <FontAwesomeIcon icon={faCircleExclamation} /> Failed to fetch user
            details{" "}
          </>
        );
        setLoading(false);
  ...
  const handleDeleteAccount = async () => {
    try {
      ...
      setError(
        <>
          {" "}
          <FontAwesomeIcon icon={faCircleExclamation} /> Failed to initiate
          account deletion{" "}
        </>
      );
    }
