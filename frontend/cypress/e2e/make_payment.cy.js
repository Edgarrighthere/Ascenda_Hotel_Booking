...
describe('Stripe Component Test', () => {
    it('Validate Successful Payment and Raedirect to Confirmation Page', () => {
        // Visit the home page
        cy.visit('/');
        ...
        // Mock the Stripe checkout session creation response
    cy.intercept('POST', 'http://localhost:5000/checkout', {
        statusCode: 200,
        body: { id: 'cs_test_123456789' },
      }).as('checkout');
      ...
      cy.wait('@checkout').then((interception) => {
        ...
        // Mock the Stripe redirection to checkout
        cy.window().then((win) => {
          ...
          stripe.redirectToCheckout = cy.stub().resolves({ success_url: `http://localhost:3000/complete/cs_test_123456789` });
  
          // Simulate the redirect to the success URL
          cy.visit(`http://localhost:3000/complete/cs_test_123456789`);
        });
      });
  
      // Verify redirection to the confirmation page
